...
const PowerDetail = (props) => {
  ...
  useEffect(() => {
    if (series) {
      ...
      setLoad(series[activeSeries - 1].load);
  ...
  return (
    <div className="pwa-mobile-container">
      ...
      <div>
        <div className="weight-block px40 pt50">
          <div className="flex-x space-between">
            <div className="plus-icon" onClick={minusLoad}>
              <img src={minusIcon} alt="" />
            </div>
            <div className="text-center light--text an-14 bold-text">
              <img src={loadImage} alt="recovery" className="mb10" />
              <div>{`${load} %`}</div>
            </div>
            <div className="plus-icon" onClick={plusLoad}>
              <img src={plusIcon} alt="" />
            </div>
          </div>
          <Slider
            tipFormatter={(value) => (
              <span className="an-16 bold-text">{`${value}  %`}</span>
            )}
